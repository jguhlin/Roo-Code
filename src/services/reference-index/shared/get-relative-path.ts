 ...
 */
export function generateNormalizedAbsolutePath(filePath: string, rootPath?: string): string {
	const baseRoot = rootPath ?? getWorkspacePath()
	// Resolve the path to make it absolute if it's relative
	const resolvedPath = path.resolve(baseRoot, filePath)
 ...
 */
export function generateRelativeFilePath(normalizedAbsolutePath: string, rootPath?: string): string {
	const baseRoot = rootPath ?? getWorkspacePath()
	// Generate the relative path
	const relativePath = path.relative(baseRoot, normalizedAbsolutePath)
