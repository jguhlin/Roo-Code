...
import { ReferenceIndexStateManager, IndexingState } from "./state-manager"
 ...
 */
export class ReferenceIndexOrchestrator {
	private _fileWatcherSubscriptions: vscode.Disposable[] = []
	private _isProcessing: boolean = false

	constructor(
		private readonly configManager: ReferenceIndexConfigManager,
		private readonly stateManager: ReferenceIndexStateManager,
	...
	private async _startWatcher(): Promise<void> {
		...
		try {
			...
			this._fileWatcherSubscriptions = [
				...
				this.fileWatcher.onDidFinishBatchProcessing((summary: BatchProcessingSummary) => {
					if (summary.batchError) {
						console.error(`[ReferenceIndexOrchestrator] Batch processing failed:`, summary.batchError)
					} else {
			...
			console.error("[ReferenceIndexOrchestrator] Failed to start file watcher:", error)
			throw error
		}
	}
	...
	public async startIndexing(): Promise<void> {
		if (!this.configManager.isFeatureConfigured) {
			...
			console.warn("[ReferenceIndexOrchestrator] Start rejected: Missing configuration.")
			return
		}
		...
		if (
			...
			console.warn(
				`[ReferenceIndexOrchestrator] Start rejected: Already processing or in state ${this.stateManager.state}.`,
			)
			return
		}
		...
		try {
			...
			const result = await this.scanner.scanDirectory(
				...
				(batchError: Error) => {
					console.error(
						`[ReferenceIndexOrchestrator] Error during initial scan batch: ${batchError.message}`,
						batchError,
					)
			...
			console.error("[ReferenceIndexOrchestrator] Error during indexing:", error)
			try {
				await this.vectorStore.clearCollection()
			} catch (cleanupError) {
				console.error("[ReferenceIndexOrchestrator] Failed to clean up after error:", cleanupError)
			}
		...
		try {
			...
			try {
				if (this.configManager.isFeatureConfigured) {
					...
					console.warn(
						"[ReferenceIndexOrchestrator] Service not configured, skipping vector collection clear.",
					)
				}
			} catch (error: any) {
				console.error("[ReferenceIndexOrchestrator] Failed to clear vector collection:", error)
